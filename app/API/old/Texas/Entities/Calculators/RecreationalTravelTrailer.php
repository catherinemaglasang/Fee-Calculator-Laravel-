...
use Thirty98\API\Texas\Entities\Repositories\RecreationalTravelTrailerRepository;

class RecreationalTravelTrailer extends Calculator
{
    const STATE_CODE = 'TX';
    ...
    {
        $this->calculateFees(new RecreationalTravelTrailerRepository($this->calculatorInput->getRequest()));
     ...
     * Calculate all fees.
     *
     * @param RecreationalTravelTrailerRepository $repository
     */
    private function calculateFees(RecreationalTravelTrailerRepository $repository)
    {
        $this->fees = [
            'EMISSION_FEE' => 0,
