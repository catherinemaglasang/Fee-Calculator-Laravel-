...
class TruckTractorTruckTractorRepository implements CalculatorRepositoryInterface
{
    private $ttltypes = [];
    ...
    public function getLicenseFee()
    {
        ...
        if ($type_of_plate == false || $date_of_sale == false || $gvwr == false) {
            $report = [
            ...
            $result_arr['msg'] = "Type of Plate, Date of Sale and GVW/GVWR must be included in the request parameters. " . json_encode($report);

            return $result_arr;
        }

        if ($type_of_plate == 'Truck Plate') {
    ...
    public function getLicenseTruckWeightFees($gvwr, $type, $category, $type_of_plate, $date_of_sale)
    {
        ...
        if ($gvwr >= 10000) {
            ...
            if ($july_diff_count > 0) {
                ...
                $prorated_license_fee = $license_fee * $license_multiplier;
